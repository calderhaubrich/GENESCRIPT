...
print('q0      = {0:9.5g}'.format(float(q_spl(r))))
print('shat    = {0:9.5g} !(defined as r/q*dq_dr)'.format(
        r/float(q_spl(r))*q_spl.derivatives(r)[1]))
# print 'shat=%9.5g (defined as (psi-psiax)/q*dq_dpsi)' %((psi-linpsi[0])/q_spl(
# r)*q_spl_psi.derivatives(psi)[1])
print('amhd    = {0:9.5g}'.format(float(amhd_spl(r))))
# print 'amhd_Miller=%9.5g' %amhd_Miller[ind]
# print test[ind]
print('drR     = {0:9.5g}'.format(float(drR_spl(r))))
print('drZ     = {0:9.5g}'.format(float(drZ_spl(r))))
print('kappa   = {0:9.5g}'.format(float(kappa_spl(r))))
print('s_kappa = {0:9.5g}'.format(kappa_spl.derivatives(r)[1]*r/float(kappa_spl(r))))
print('delta   = {0:9.5g}'.format(float(delta_spl(r))))
print('s_delta = {0:9.5g}'.format(
        delta_spl.derivatives(r)[1]*r/np.sqrt(1 - float(delta_spl(r)) ** 2)))
print('zeta    = {0:9.5g}'.format(float(zeta_spl(r))))
...
print('Z0          = {0:9.5g}'.format(float(Zavg_spl(r))))
...
#Calder Edits#
R_tm01 = np.empty((pw, ntheta), dtype=float)
Z_tm01 = np.empty((pw, ntheta), dtype=float)
poi01 = find(float(0.1), rho_tor)
stenc01 = range(poi01 - pw//2, poi01 + pw//2)
if stenc01[0] < 1:
    stenc01 = [stenc01[i] + 1 - stenc01[0] for i in range(len(stenc01))]
if stenc01[-1] > nw - 1:
    stenc01 = [stenc01[i] - (stenc01[-1] - nw + 1) for i in range(len(stenc01))]
in01 = poi01 - stenc01[0]
...
for i in stenc01:
    ...
    R_int = interp1d(theta_mod_ext, R_extended, kind=interpol_order)
    Z_int = interp1d(theta_mod_ext, Z_extended, kind=interpol_order)
    R_tm01[imod] = R_int(theta_arr)
    Z_tm01[imod] = Z_int(theta_arr)

R_tm02 = np.empty((pw, ntheta), dtype=float)
Z_tm02 = np.empty((pw, ntheta), dtype=float)
poi02 = find(float(0.2), rho_tor)
stenc02 = range(poi02 - pw//2, poi02 + pw//2)
if stenc02[0] < 1:
    stenc02 = [stenc02[i] + 1 - stenc02[0] for i in range(len(stenc02))]
if stenc02[-1] > nw - 1:
    stenc02 = [stenc02[i] - (stenc02[-1] - nw + 1) for i in range(len(stenc02))]
in02 = poi02 - stenc02[0]
...
for i in stenc02:
    ...
    R_int = interp1d(theta_mod_ext, R_extended, kind=interpol_order)
    Z_int = interp1d(theta_mod_ext, Z_extended, kind=interpol_order)
    R_tm02[imod] = R_int(theta_arr)
    Z_tm02[imod] = Z_int(theta_arr)

R_tm03 = np.empty((pw, ntheta), dtype=float)
Z_tm03 = np.empty((pw, ntheta), dtype=float)
poi03 = find(float(0.3), rho_tor)
stenc03 = range(poi03 - pw//2, poi03 + pw//2)
if stenc03[0] < 1:
    stenc03 = [stenc03[i] + 1 - stenc03[0] for i in range(len(stenc03))]
if stenc03[-1] > nw - 1:
    stenc03 = [stenc03[i] - (stenc03[-1] - nw + 1) for i in range(len(stenc03))]
in03 = poi03 - stenc03[0]
...
for i in stenc03:
    ...
    R_int = interp1d(theta_mod_ext, R_extended, kind=interpol_order)
    Z_int = interp1d(theta_mod_ext, Z_extended, kind=interpol_order)
    R_tm03[imod] = R_int(theta_arr)
    Z_tm03[imod] = Z_int(theta_arr)

R_tm04 = np.empty((pw, ntheta), dtype=float)
Z_tm04 = np.empty((pw, ntheta), dtype=float)
poi04 = find(float(0.4), rho_tor)
stenc04 = range(poi04 - pw//2, poi04 + pw//2)
if stenc04[0] < 1:
    stenc04 = [stenc04[i] + 1 - stenc04[0] for i in range(len(stenc04))]
if stenc04[-1] > nw - 1:
    stenc04 = [stenc04[i] - (stenc04[-1] - nw + 1) for i in range(len(stenc04))]
in04 = poi04 - stenc04[0]
...
for i in stenc04:
    ...
    R_int = interp1d(theta_mod_ext, R_extended, kind=interpol_order)
    Z_int = interp1d(theta_mod_ext, Z_extended, kind=interpol_order)
    R_tm04[imod] = R_int(theta_arr)
    Z_tm04[imod] = Z_int(theta_arr)

R_tm05 = np.empty((pw, ntheta), dtype=float)
Z_tm05 = np.empty((pw, ntheta), dtype=float)
poi05 = find(float(0.5), rho_tor)
stenc05 = range(poi05 - pw//2, poi05 + pw//2)
if stenc05[0] < 1:
    stenc05 = [stenc05[i] + 1 - stenc05[0] for i in range(len(stenc05))]
if stenc05[-1] > nw - 1:
    stenc05 = [stenc05[i] - (stenc05[-1] - nw + 1) for i in range(len(stenc05))]
in05 = poi05 - stenc05[0]
...
for i in stenc05:
    ...
    R_int = interp1d(theta_mod_ext, R_extended, kind=interpol_order)
    Z_int = interp1d(theta_mod_ext, Z_extended, kind=interpol_order)
    R_tm05[imod] = R_int(theta_arr)
    Z_tm05[imod] = Z_int(theta_arr)

R_tm06 = np.empty((pw, ntheta), dtype=float)
Z_tm06 = np.empty((pw, ntheta), dtype=float)
poi06 = find(float(0.6), rho_tor)
stenc06 = range(poi06 - pw//2, poi06 + pw//2)
if stenc06[0] < 1:
    stenc06 = [stenc06[i] + 1 - stenc06[0] for i in range(len(stenc06))]
if stenc06[-1] > nw - 1:
    stenc06 = [stenc06[i] - (stenc06[-1] - nw + 1) for i in range(len(stenc06))]
in06 = poi06 - stenc06[0]
...
for i in stenc06:
    ...
    R_int = interp1d(theta_mod_ext, R_extended, kind=interpol_order)
    Z_int = interp1d(theta_mod_ext, Z_extended, kind=interpol_order)
    R_tm06[imod] = R_int(theta_arr)
    Z_tm06[imod] = Z_int(theta_arr)

R_tm07 = np.empty((pw, ntheta), dtype=float)
Z_tm07 = np.empty((pw, ntheta), dtype=float)
poi07 = find(float(0.7), rho_tor)
stenc07 = range(poi07 - pw//2, poi07 + pw//2)
if stenc07[0] < 1:
    stenc07 = [stenc07[i] + 1 - stenc07[0] for i in range(len(stenc07))]
if stenc07[-1] > nw - 1:
    stenc07 = [stenc07[i] - (stenc07[-1] - nw + 1) for i in range(len(stenc07))]
in07 = poi07 - stenc07[0]
...
for i in stenc07:
    ...
    R_int = interp1d(theta_mod_ext, R_extended, kind=interpol_order)
    Z_int = interp1d(theta_mod_ext, Z_extended, kind=interpol_order)
    R_tm07[imod] = R_int(theta_arr)
    Z_tm07[imod] = Z_int(theta_arr)

R_tm08 = np.empty((pw, ntheta), dtype=float)
Z_tm08 = np.empty((pw, ntheta), dtype=float)
poi08 = find(float(0.8), rho_tor)
stenc08 = range(poi08 - pw//2, poi08 + pw//2)
if stenc08[0] < 1:
    stenc08 = [stenc08[i] + 1 - stenc08[0] for i in range(len(stenc08))]
if stenc08[-1] > nw - 1:
    stenc08 = [stenc08[i] - (stenc08[-1] - nw + 1) for i in range(len(stenc08))]
in08 = poi08 - stenc08[0]
...
for i in stenc08:
    ...
    R_int = interp1d(theta_mod_ext, R_extended, kind=interpol_order)
    Z_int = interp1d(theta_mod_ext, Z_extended, kind=interpol_order)
    R_tm08[imod] = R_int(theta_arr)
    Z_tm08[imod] = Z_int(theta_arr)

R_tm09 = np.empty((pw, ntheta), dtype=float)
Z_tm09 = np.empty((pw, ntheta), dtype=float)
poi09 = find(float(0.9), rho_tor)
stenc09 = range(poi09 - pw//2, poi09 + pw//2)
if stenc09[0] < 1:
    stenc09 = [stenc09[i] + 1 - stenc09[0] for i in range(len(stenc09))]
if stenc09[-1] > nw - 1:
    stenc09 = [stenc09[i] - (stenc09[-1] - nw + 1) for i in range(len(stenc09))]
in09 = poi09 - stenc09[0]
...
for i in stenc09:
    ...
    R_int = interp1d(theta_mod_ext, R_extended, kind=interpol_order)
    Z_int = interp1d(theta_mod_ext, Z_extended, kind=interpol_order)
    R_tm09[imod] = R_int(theta_arr)
    Z_tm09[imod] = Z_int(theta_arr)

R_tm99 = np.empty((pw, ntheta), dtype=float)
Z_tm99 = np.empty((pw, ntheta), dtype=float)
poi99 = find(float(0.99), rho_tor)
stenc99 = range(poi99 - pw//2, poi99 + pw//2)
if stenc99[0] < 1:
    stenc99 = [stenc99[i] + 1 - stenc99[0] for i in range(len(stenc99))]
if stenc99[-1] > nw - 1:
    stenc99 = [stenc99[i] - (stenc99[-1] - nw + 1) for i in range(len(stenc99))]
in99 = poi99 - stenc99[0]
...
for i in stenc99:
    ...
    R_int = interp1d(theta_mod_ext, R_extended, kind=interpol_order)
    Z_int = interp1d(theta_mod_ext, Z_extended, kind=interpol_order)
    R_tm99[imod] = R_int(theta_arr)
    Z_tm99[imod] = Z_int(theta_arr)

figu = plt.figure()
plt.plot(R_tm01[in01], Z_tm01[in01], 'k-', lw=1, label = '$\\rho_{tor}$ = 0.1')
plt.plot(R_tm02[in02], Z_tm02[in02], 'k-', lw=1, label = '$\\rho_{tor}$ = 0.2')
plt.plot(R_tm03[in03], Z_tm03[in03], 'k-', lw=1, label = '$\\rho_{tor}$ = 0.3')
plt.plot(R_tm04[in04], Z_tm04[in04], 'k-', lw=1, label = '$\\rho_{tor}$ = 0.4')
plt.plot(R_tm05[in05], Z_tm05[in05], 'k-', lw=1, label = '$\\rho_{tor}$ = 0.5')
plt.plot(R_tm06[in06], Z_tm06[in06], 'k-', lw=1, label = '$\\rho_{tor}$ = 0.6')
plt.plot(R_tm07[in07], Z_tm07[in07], 'k-', lw=1, label = '$\\rho_{tor}$ = 0.7')
plt.plot(R_tm08[in08], Z_tm08[in08], 'k-', lw=1, label = '$\\rho_{tor}$ = 0.8')
plt.plot(R_tm09[in09], Z_tm09[in09], 'k-', lw=1, label = '$\\rho_{tor}$ = 0.9')
plt.plot(R_tm99[in99], Z_tm99[in99], 'k-', lw=1, label = '$\\rho_{tor}$ = 0.99')
plt.xlabel('$R$/m', fontsize=16)
plt.ylabel('$Z$/m', fontsize=16)
plt.xticks(fontsize=12)
plt.yticks(fontsize=12)
axies = plt.gca()
axies.set_aspect(1.1)
#axies.axes.get_xaxis().set_visible(False)
#axies.axes.get_yaxis().set_visible(False)
plt.show()
