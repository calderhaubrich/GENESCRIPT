...
def read_csv(csvfpath):
    with open(csvfpath, mode='r') as csvfid:
...
def read_chease(cheasefpath,setParam={},Normalized=False,**kwargs):
    ...
    if   h5py.is_hdf5(cheasefpath):
         CHEASEdata = read_chease_hdf(cheasefpath,setParam,Normalized,**kwargs)
    else:
         CHEASEdata = read_chease_dat(cheasefpath,setParam,Normalized,**kwargs)

    return CHEASEdata

def read_chease_dat(cheasefpath,setParam={},Normalized=False,**kwargs):
    if os.path.isfile(cheasefpath) == False:
       errorFunc = traceback.extract_stack(limit=2)[-2][3]
       errorLine = traceback.extract_stack(limit=2)[-2][1]
       errorFile = traceback.extract_stack(limit=2)[-2][2]
...
def read_chease_hdf(cheasefpath,setParam={},Normalized=False,**kwargs):
    if os.path.isfile(cheasefpath) == False:
       errorFunc = traceback.extract_stack(limit=2)[-2][3]
       errorLine = traceback.extract_stack(limit=2)[-2][1]
       errorFile = traceback.extract_stack(limit=2)[-2][2]
...
def read_efit(efitfpath,setParam={},Normalized=False,**kwargs):
    if Normalized:
       EFITdata = read_eqdsk(eqdskfpath=efitfpath,setParam=setParam,Normalized=True,**kwargs)
    else:
       EFITdata = read_eqdsk(eqdskfpath=efitfpath,setParam=setParam,Normalized=False,**kwargs)
...
def read_profiles(profilesfpath,setParam={},Zeffprofile=True,**kwargs):
    ...
    PROFILESdata['omeg']    = profiles['omeg'][:]
    ...
    if   interpflag:
         if   rhopsiflag:
              ...
              PROFILESdata['omeg']     = mathtools.interp(PROFILESdata['PSIN'],PROFILESdata['omeg'],psi)
              if 'tb' in profiles:
              ...
              PROFILESdata['omeg']     = mathtools.interp(PROFILESdata['PSIN'],PROFILESdata['omeg'],psi,phi,phi)
              if 'tb' in profiles:
